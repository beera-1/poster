...
async def check_url(session, url):
    """Check if the given image URL exists on VivaMax CDN"""
    try:
...
def extract_title_from_page(url):
    """Fetch HTML & extract title text using BeautifulSoup"""
    try:
...
async def get_vivamax_posters(viva_url):
    """Main function to extract poster URLs"""
    ...
    # Build poster URLs and verify existence
    results = []
